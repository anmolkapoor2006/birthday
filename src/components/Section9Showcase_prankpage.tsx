...
import { RotateCcw, Sparkles, Laugh, Heart } from 'lucide-react';
  ...
  return (
    <ReactLenis root options={{ duration: 1.2, lerp: 0.08, smoothWheel: true }}>
      <div className={cn('relative w-full select-none', className)} ref={container}>
        ...
        {/* 🦎 CHIPKALI JUMPSCARE PRANK MODAL 🦎 */}
        <AnimatePresence>
          {showPrankModal && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[99999] bg-black/95 flex flex-col items-center justify-center p-4 select-none overflow-hidden"
            >
              {/* Full Screen Lizard Jumpscare Container */}
              <motion.div
                initial={{ scale: 0.1, rotate: -25, opacity: 0 }}
                animate={{
                  scale: [0.1, 1.35, 1, 1.15, 1],
                  rotate: [0, 8, -8, 4, 0],
                  x: [0, -15, 15, -8, 0],
                  opacity: 1,
                }}
              ...
              >
                {/* Real High-Resolution Lizard Jumpscare Image */}
                <div className="relative w-80 h-80 md:w-[420px] md:h-[420px] rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(239,68,68,0.7)] border-4 border-red-500/80 my-2">
                  <motion.div
                    animate={{
                  ...
                  >
                    <Image
                      src="/images/real_chipkali.png"
                      alt="Real Chipkali Lizard"
                      fill
                      className="object-cover object-center"
                      sizes="(max-width: 768px) 320px, 420px"
                      priority
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-red-500/20 pointer-events-none" />
                  </motion.div>
                </div>

                {/* Funny Prank Heading */}
                <h2 className="font-script text-4xl md:text-6xl text-emerald-400 drop-shadow-md mb-2 animate-bounce">
                  CHIPKALI! 🦎😜
                </h2>
                
                <p className="font-sans text-sm md:text-base text-gray-200 font-medium mb-2 px-4 leading-relaxed">
                  Darr gayi na Golu? Hahahahaha! 🤣
                </p>

                <p className="font-sans text-xs text-pink-300 font-light mb-6 px-6 leading-relaxed">
                  Itna emotional hone ke baad thoda darna bhi zaroori hai! I love you so much chipkali 💖
                </p>
                ...
                <motion.button
                  ...
                  className="flex items-center space-x-2 px-8 py-3.5 bg-gradient-to-r from-pink-500 via-rose-500 to-amber-500 hover:from-pink-600 hover:to-amber-600 text-white font-bold text-sm rounded-full shadow-2xl cursor-pointer border border-pink-300/40"
                >
                  <RotateCcw className="w-4 h-4" />
                  <span>Acha Chalo, Replay Surprise Now 💖</span>
                </motion.button>
              </motion.div>
            </motion.div>
          )}
