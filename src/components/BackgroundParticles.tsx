...
// Generate particles once at module level (client-only bundle) to avoid
// SSR mismatch and the useEffect→setState cascading render pattern.
const PARTICLES: Particle[] = Array.from({ length: 28 }).map((_, i) => ({
  id: i,
  type: (i % 3 === 0 ? 'petal' : i % 3 === 1 ? 'heart' : 'sparkle') as Particle['type'],
  x: (i * 37.3 + 11) % 100,           // deterministic spread across width
  size: 10 + (i * 7) % 14,             // 10px to 24px
  duration: 20 + (i * 11) % 25,        // 20s to 45s
  delay: -((i * 13) % 45),             // stagger start positions
  drift: ((i * 17) % 80) - 40,         // -40px to 40px
}));
