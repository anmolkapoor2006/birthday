...
import { playPaperSlideSound } from '@/utils/audio';
...
export default function Section1Envelope({ onOpen }: Section1EnvelopeProps) {
  ...
  const handleSealClick = (e?: React.MouseEvent) => {
    ...
    // 1. Open the envelope flap after seal breaks
    setTimeout(() => {
      setIsFlapOpen(true);
      
      // 2. Slide the card/letter up
      setTimeout(() => {
        setIsLetterOut(true);
        playPaperSlideSound();
        
        // 3. Trigger transition to Section 2
        setTimeout(() => {
          onOpen();
        }, 800);
      }, 500);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] w-full px-4 relative z-10 select-none">
      ...
      <motion.div
        ...
        className="relative w-72 h-48 md:w-96 md:h-64 cursor-pointer group"
      ...
      >
        ...
        <AnimatePresence>
          {!isSealBroken && (
            ...
            >
              {/* Inner stamp ring */}
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border border-white/40 flex items-center justify-center pointer-events-none">
